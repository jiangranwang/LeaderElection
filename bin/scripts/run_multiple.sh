  ...
  for drop_rate in ${drop_rates[@]}; do
    ...
    for (( i=1; i<=$num_run; i++ )); do
      ...
      for algo in ${algos[@]}; do
        ...
        if [ ! -d "../analyze/results/${num_server}/${drop_rate}/algo${algo}/" ]; then
          mkdir -p "../analyze/results/${num_server}/${drop_rate}/algo${algo}/"
        fi
        ./run.sh $config_name
        mv stats.json "../analyze/results/${num_server}/${drop_rate}/algo${algo}/stats_${i}.json"
      done
    done
    idx=$(( $idx + 1 ))
  done
